...
/* 
    Opcodes may be composed of 1 byte, 2 bytes or 3 bytes.
    Registers: A, B, C, D, E, H, L. PC (program counter) and SP (stack pointer). Some registers work in pairs: BC, DE, HL.
*/

int disassemble(unsigned char* codebuffer, int pc) {
    OPCODE opcode = &codebuffer[pc];
    int opbytes = 1;
	printf("0x%04x  ", pc);
    switch (*opcode) {
        case 0x00: printf("NOP"); break;
        case 0x01: printf("LXI    B, #$%02x%02x", opcode[2], opcode[1]); opbytes = 3; break; // code[2] then code[1] -> little endian
        ...
        case 0xdb: printf("IN     #$%02x"); opbytes = 2; break;
    ...
    return opbytes; // Returns how many bytes compose the single opcode -> increments the program counter
}
...
int main(int argc, char** argv) {
    ...
    if (game == NULL) {
        ...
        return 1; // Exit with 1 status code
    }
    
    fseek(game, 0L, SEEK_END); // Position the pointer at the end of the file
    int size = ftell(game); // ftell returns the size of the file by returning the position of the pointer
    ...
    while(programCounter < size) {
        programCounter += disassemble(buffer, programCounter); // Increments the program counter by the number of bytes composing the opcode
    }
